$(function() {
    ...
    images.forEach(image => {
        image.innerHTML = `<div class='need-permission'><span>Bitte aktiviere das Anzeigen von Bildern, um diesen Inhalt zu laden.</span></div>`;
    });
});
...
function loadImages() {
    ...
    images.forEach(imageContainer => {
        ...
        if(src.startsWith('http') || src.startsWith('www.') || /[a-zA-Z]{1,}.(de|com|net|eu|co|tv|fr|nrw)/.test(src)) {
            imageContainer.innerHTML += `<span class='image-source'>Quelle: ${src}</span>`;
        }
